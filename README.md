...
- [NUC 实机接入](robot/README.md)：环境安装脚本、逐项检查（环境/驱动/相机）与单关节小幅点动；尚未在实机上运行。
...
- 本项目只做初赛，报名已完成（USER-RULE-20260923）；决赛规则仅作资料保留。
...
先在本机准备三方案共用的视觉采集和离线识别，同时核验电池实物与 S3 末端接口。方案一检查提手通道、钩具承载、止退和脱钩路径；方案二检查侧面平整与气密条件、真空建立与保持、侧向承载和释放动作；方案三检查卡环与提手/本体的配合、锁止、承载和释放路径。三方案先做台架对比，再验证机械臂搬运、单块放置及三块序列。D435i 采用末端刚性固定，分别检查工具遮挡和布线；换工具须重新标定相应 TCP，相机相对末端的固定关系改变时重新进行手眼标定。S3 控制柜/软件版本仍待明确。具体见硬件与传统视觉方案及末端三方案对比。

2026-09-23 用户确定实机使用 ROS 2 Humble（USER-HW-20260923、USER-HW-20260923-02），与仿真同一发行版，部署在 NUC 上的原生 Ubuntu 22.04 而非 WSL2（USER-HW-20260923-03、USER-HW-20260923-04），当前开发电脑用于开发和跑仿真，并经 SSH 隧道远程查看 NUC（USER-HW-20260923-06、USER-HW-20260923-07）；S3 用 `aubo_ros2_driver`（USER-HW-20260923-08），与控制柜的兼容性未验证；NUC、S3、D435i 均已到位（USER-HW-20260923-11），D435i 用 realsense-ros（USER-HW-20260923-09），运动规划在实机和仿真都用 MoveIt（USER-HW-20260923-10、USER-SIM-20260923，仿真尚未接入）；NUC 型号与接口、驱动在实机上的验证尚未完成，见 Q02、Q05、Q17。
